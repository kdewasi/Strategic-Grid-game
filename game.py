...
class Board:
    def _init_(self,width,height, p1_sprites, p2_sprites):
        self.width = width
        self.height = height
    ...
    def create_custom_board():
        custom_width = int(input("Enter the width of the board: "))
        custom_height = int(input("Enter the height of the board: "))
        ...
        return Board(custom_width, custom_height, p1_sprites, p2_sprites)
...
def undo_last_move(self):
        if self.turn > 0:
            last_player = 1 if self.turn % 2 == 0 else -1
            for row in range(self.height):
                for col in range(self.width):
...
def save_game_state(self):
        with open('saved_game_state.pkl', 'wb') as f:
            pickle.dump(self, f)
